...
struct left_type* new_left_type_ptr() {
    struct left_type* res = (struct left_type*)malloc(sizeof(struct left_type));
    if (res == NULL) {
        printf("Failure in malloc.\n");
        exit(0);
    }
...
struct glob_item* new_glob_item_ptr() {
    struct glob_item* res = (struct glob_item*)malloc(sizeof(struct glob_item));
    if (res == NULL) {
        printf("Failure in malloc.\n");
        exit(0);
    }
...
struct enum_ele_list* TENil() { return NULL; }
...
struct glob_item_list* TGNil() { return NULL; }
...
struct print_ret* OrigTypeReturn(char* name) {
    struct print_ret* res = (struct print_ret*)malloc(sizeof(struct print_ret));
